...
def get_discord_login_url():
    response = requests.get("http://localhost:8000/discord/login")
...
def get_discord_user_details(access_token):
    ...
    response = requests.get("http://localhost:8000/discord/user", headers=headers)
    if response.status_code == 200:
        minified_user = {
...
def check_discord_exists(discord_id):
    response = requests.get(f"http://localhost:8000/discord/{discord_id}")
    if response:
...
def save_user_data(puuid, discord_id, discord_username):
    response = requests.post(f"http://localhost:8000/valorant/rank/{puuid}/{discord_id}/{discord_username}/")
    if response.status_code == 200:
