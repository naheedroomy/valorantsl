...
def fetch_leaderboard_data(page, page_size):
    response = requests.get(f"{VALORANTSL_API_URL}/valorant/leaderboard?page={page}&page_size={page_size}")
...
def get_discord_login_url():
    response = requests.get(f"{VALORANTSL_API_URL}/discord/login")
...
def get_discord_user_details(access_token):
    ...
    response = requests.get(f"{VALORANTSL_API_URL}/discord/user", headers=headers)
    if response.status_code == 200:
        minified_user = {
...
def check_discord_exists(discord_id):
    response = requests.get(f"{VALORANTSL_API_URL}/discord/{discord_id}")
    if response:
...
def save_user_data(puuid, discord_id, discord_username):
    response = requests.post(f"{VALORANTSL_API_URL}/valorant/rank/{puuid}/{discord_id}/{discord_username}/")
    if response.status_code == 200:
